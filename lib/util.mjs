...
/** A committed row names no local absolute path: the repo root becomes `.`, the home directory `~`. */
export function scrub(json) {
  const esc = (s) => JSON.stringify(s).slice(1, -1).replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  return json.replace(new RegExp(esc(ROOT) + '(?=/|")', 'g'), '.').replace(new RegExp(esc(os.homedir()) + '(?=/|")', 'g'), '~');
}
export function appendJsonl(file, row) {
  fs.mkdirSync(path.dirname(file), { recursive: true });
  fs.appendFileSync(file, scrub(JSON.stringify(row)) + '\n');
}
