...
base defaulting to `https://peerinfinity.github.io/tmt-loader/`), `loader_commit`, the upstream commit the loader
copied and the loader's per-file license verdict. The file is rewritten whole on every run, so two runs are
byte-identical; a manifest that joins no census repo is printed, not written.

`rank.mjs` joins the rows on `full_name` and carries `loader_url`, `loader_id` and `loader_commit` into
`results/table.json`, a `loader` column in `results/SUMMARY.md` and on the page — for every ranked row the loader
hosts, including rows that also have a live page — and names the loader commit and base in SUMMARY's provenance line
