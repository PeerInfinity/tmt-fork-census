...
| `base` (Base game) | The calibration tree this game is built on: its layer-id set contains that tree's, so it is that game plus added or changed content (STAGES.md, Family collapse (c)). | PTR, TMT or upgrade-land-tmt; empty when it is not built on one |
